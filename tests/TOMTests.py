...
class TOMTests(unittest.TestCase):
    ...
    def test_access_without_JWT(self):
        dummy_data = {"consent_id": "TEST11091113454114545512", "reason": "pdf"}
        header = {"Content-Type": "application/json"}
        response = requests.post(self.JWT_URI_TEST, json=dummy_data)
        response = int(re.findall(r"\d+", str(response))[0])
        self.assertEqual(response, 500)

    def test_access_with_valid_JWT(self):
        login = {"password": "aa", "username": "aside"}
        response = requests.post(self.JWT_LOGIN, json=login)
    ...
    def test_access_with_valid_token_unauthorised_role(self):
        consent = {
            "Agents": [{"id": "60a55c9dd79bc75769804113ea", "role": "controller"}],
            "DataProcessing": [
                "storage",
            ],
        ...
        login = {"password": "123das", "username": "luhapiaccessid"}
        response = requests.post(self.JWT_LOGIN, json=login)
        ...
        response_CREATE_CS = requests.post(
            "http://127.0.0.1:5001/consent/create",
            json=consent,
            headers={
                "Content-Type": "application/json",
                "Authorization": "Bearer {}".format(access_token),
    ...
    def test_minimisation_test_fail(self):
        ...
        response_CREATE_CS_fail = requests.post(
            "http://127.0.0.1:5001/consent/create",
            json=consent,
            headers={
                "Content-Type": "application/json",
    ...
    def test_minimisation_test_pass(self):
        ...
        response_CREATE_CS_pass = requests.post(
            "http://127.0.0.1:5001/consent/create",
            json=consent,
            headers={
                "Content-Type": "application/json",
        ...
        eval_true = (
            True if "CONSENT_CREATION_SUCCESS" in act_response["decision"] else False
        )
