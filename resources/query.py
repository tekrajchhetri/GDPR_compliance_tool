...
class NestedSchema(Schema):
    ConsentID = fields.Dict(required=True, keys=fields.Str(), values=fields.Str())
...
class QueryConsentIDByConsentProviderID(MethodResource, Resource):
    ...
    def get(self, consentprovider_id):
        query = QueryEngine()
        resp = json.loads(
            query.select_query_gdb(
                consentProvidedBy=consentprovider_id,
                purpose=None,
                dataProcessing=None,
                dataController=None,
                dataRequester=None,
