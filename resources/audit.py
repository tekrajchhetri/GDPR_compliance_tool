...
from core.security.JWTDecorator import ccc_required, access_to_all, spm_required
import json
...
class AuditConsent(MethodResource, Resource):
    ...
    def get(self, consent_id, level_of_details):
        auditInstance = Audit()
        response = auditInstance.audit__consent(
            consent_id=consent_id, level_of_details=level_of_details
        )
...
class AuditDataProvider(MethodResource, Resource):
    ...
    def get(self, data_provider_id, level_of_details):
        auditInstance = Audit()
        response = auditInstance.audit_all_consent_by_dp(
            data_provider_id=data_provider_id, level_of_details=level_of_details
        )
