...
class ComplianceEngine(QueryEngine, DateHelper):
    def __init__(self):
        super().__init__()
        self.TRIGGER_URL_NOTIFY = (
            "https://tektestapi.herokuapp.com/notify"  # dummy test URL to be replaced
        )
    ...
    def broken_consent(self, consentID, reason_for_logging):
        ...
        if self.check_active_granted_consent(consentID=consentID):
            respone = self.post_sparql(
                ...
                query=self.revoke_broken_consent_query(
                    consentID=consentID, type="BROKEN_CONSENT"
                ),
                consent_id_for_logging=consentID,
                reason_for_logging=reason_for_logging,
                type="broken_consent",
            )
            updateRevokations = NGAC().updateRevokation(
                token="admin_token",
    ...
    def get_consent_data(self, level="all", consentID=None, consentProvidedBy=None):
        if level == "all":
            ...
            all_consent = self.select_query_gdb(
                additionalData="consent_for_compliance_consent", consentID=consentID
            )
            return json.loads(all_consent)
        elif level == "dataprovider":
            all_consent = self.select_query_gdb(
                additionalData="consent_for_compliance_dataprovider",
    ...
    def notify(self, data):
        ...
        r = requests.post(
            self.TRIGGER_URL_NOTIFY, data=json.dumps(data), headers=header
        )
    ...
    def get_processing_details_from_controller(self, whoseInfoDict):
        header = {"Content-Type": "application/json"}
        r = requests.post(
            self.TRIGGER_URL_CONTROLLER, data=json.dumps(whoseInfoDict), headers=header
        )
        msg = json.loads(r.text)
        return msg
    ...
    def compliance_check_act(self, level="all", consentID=None, consentProvidedBy=None):
        ...
        consent_datas = self.get_consent_data(
            level=level, consentID=consentID, consentProvidedBy=consentProvidedBy
        )
        for consent_data in consent_datas["results"]["bindings"]:
            cid = self.remove_uris(consent_data["ConsentID"]["value"])
            dpid = self.decrypt_data(
                self.remove_uris(consent_data["DataProvider"]["value"])
            )
            exp_date = self.remove_xst_date(
                self.decrypt_data(self.remove_uris(consent_data["Duration"]["value"]))
            )
            data_requester = self.decrypt_data(
                self.remove_uris(consent_data["DataRequester"]["value"])
            )
            data_controller = self.decrypt_data(
                self.remove_uris(consent_data["DataController"]["value"])
            )
            data_processor = self.decrypt_data(
                self.remove_uris(consent_data["DataProcessor"]["value"])
            )
            data_processing_list = [
                self.decrypt_data(self.remove_uris(litem))
                for litem in consent_data["DataProcessing"]["value"].split(",")
            ]
            purpose = self.decrypt_data(
                self.remove_uris(consent_data["Purpose"]["value"])
            )
            ...
            toCheckData = [
                ...
                for v in isAboutData
            ]
            consent_data = {
                "data": toCheckData,
                "dataprocessing": data_processing_list,
                "purpose": purpose,
            }
            for_requesting_info_from_dc_dp = {
                "ConsentID": cid,
            ...
            fromControllerDetails = self.get_processing_details_from_controller(
                for_requesting_info_from_dc_dp
            )
            ...
            if not self.has_expired(expirydate=exp_date):
                ...
                if self.has_valid_purpose(purpose, fromControllerDetails["purpose"]):
                    if self.has_processing_rights(
                        data_processing_list, fromControllerDetails["dataprocessing"]
                    ):
                        if self.is_doing_valid_data_processing(
                            toCheckData, fromControllerDetails["data"]
                        ):
                            # all_good so ask for compliance from privacy and security
                            if self.joint_compliance(
                                fordataprocessing=data_processing_list,
                                dataprovider=dpid,
                                purpose=purpose,
                                ...
                                compliance_result[
                                    "joint_compliance"
                                ] = "Failed to comply from security and privacy"
                                shouldNotify = True
                        else:
                            compliance_result[
                                "data"
                            ] = "Data (access) is different form what was consented"
                            shouldNotify = True
                    else:
                        compliance_result[
                            "dataprocessing"
                        ] = "Dataprocessing is different than what was consented"
                        shouldNotify = True
                else:
                    # purpose is invalid
                    compliance_result[
                        "purpose"
                    ] = "Data is used for different purpose {} than the requested in consent {}".format(
                        fromControllerDetails["purpose"], purpose
                    )
                    shouldNotify = True
            else:
                # revoke the consent
                ...
                compliance_result[
                    "consent_expired"
                ] = "Consent has expired so consent has been automatically revoked"
                # consent has expired
                shouldNotify = True

            if shouldNotify:
                compliance_status = {
                    "consent_info": for_requesting_info_from_dc_dp,
    ...
    def joint_compliance(
        ...
        result = NGAC().check_access_permission(
...
class ComplianceObligationNotification(smashHitmessages, Functions):
    ...
    def store_compliance_obligation_notification_status(self, received_info):
        ...
        received_info_msg = {
            ...
            "compliance_obligation_status": received_info[
                "compliance_obligation_status"
            ],
        ...
        data = {
            "received_info": received_info_msg,
            "acknowledge_message": toReturn_Message,
        }
